...
\end{lstlisting}
\lstinputlisting[frame=single,caption={[Arithmetik mit Flie\ss kommazahlen]\textbf{Achtung:} Scheme rundet bei Arithmetik mit Flie\ss kommazahlen (interne Darstellung ist binär)\\
}]{FloatArithmetik.rkt}
Ein Wert kann an einen \underline{Namen} (auch \underline{Identifier}) gebunden werden, durch \\
\lstinline!(define <id> <e>)! \hspace*{1.5cm} \auf id\zu Identifier \xspace \auf e\zu Ausdruck\\
...
\bigskip\\
\lstinputlisting[frame=single,caption={[Schlüsselwort define]Bindung von Werten an Namen}]{define.rkt}
...
\lstinputlisting[frame=single,caption={[Lambda Abstraktion] Lambda-Abstraktion}]{lambda.rkt}
